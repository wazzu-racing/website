...
function mdToHtml(markdown) {
  ...
  const clean = sanitizeHtml(rendered, {
    allowedTags: [
      ...
      'b','i','strong','em','code','pre','hr','br',
      'img','table','thead','tbody','tr','th','td','details','summary'
    ],
    allowedAttributes: {
...
function markdownToEntry(raw, filePath = '') {
  ...
  if (!title) {
    ...
    if (/^#{1,6}\s+/.test(firstLine)) {
      ...
      title = (fm.title || id).replace(/[-_]+/g, ' ').replace(/\b\w/g, (c) => String(c).toUpperCase());
    }
  }
  ...
  const tags = Array.isArray(fm.tags)
    ? fm.tags.map(String)
    : fm.tags
      ? String(fm.tags).split(',').map((s) => s.trim()).filter(Boolean)
      : [];
...
(async function main() {
  try {
    ...
    const mdFiles = names.filter((n) => n.endsWith('.md') || n.endsWith('.markdown')).map((n) => path.join(ARTICLES_DIR, n));
